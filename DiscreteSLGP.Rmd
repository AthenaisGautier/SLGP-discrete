...
for (seed in seeds) {
  for(n_freq in c(100)){
    for(n_train in c(10, 100, 1000)){
      out_compile <- sprintf("./res_synth/case1/SLGP/SLGP_seed%d_nFreq%d_n%d.RData",
                             seed, n_freq, n_train)
...
df %>%
  ...
  labs(
    x = "Parameter p",
    y = "Median value of KL(Bin, SLGP)",
  )+
  theme(legend.position = "")
ggsave("./Figures/LaplaceVSMCMC.pdf", width=5, height=4)


df_time %>%
  ggplot(aes(x = factor(n), y = t, 
...
ggsave("./Figures/Time_LaplaceVSMCMC.pdf", width=4, height=4)
```
    ...
    if(!(file.exists(outfinal))){
      for (step in seq(n_end)) {
        ...
        temp <- temp %>%
          ...
          summarise(W1 = sum(abs(estim_cdf - truth_cdf)),
                    KL_EstTrue=sum(estim_pdf * log(estim_pdf/truth_pdf)),
                    KL_TrueEst=sum(truth_pdf * log(truth_pdf/estim_pdf)),
                    TV= sum(abs(estim_pdf-truth_pdf)), 
...
```{r SLGP_compil_seq, message=FALSE, warning=FALSE, eval=FALSE}
library(ggplot2)

dir.create("./res_synth/case1_seq/datasets",
           recursive = TRUE,
           showWarnings = FALSE)
  ...
  for(strat in list_strat){
    ...
    for (step in seq(n_end)) {
      ...
      if((file.exists(out))){
        ...
        df_time <- rbind(df_time, 
                         data.frame(seed=seed,
                                    n=nrow(res$slgp@data),
                                    method="SLGP",
                                    strat=strat,
                                    t_train = res$time_sec$laplace,
                                    t_optim = res$time_sec$optim,
                                    t_pred = res$time_sec$pred))
        
        if(step == n_end && strat == "random"){
          df_data <- rbind(res$slgp@data, 
                           data.frame(output=res$t_new, p=res$p_new))
          
          write.csv(df_data,
                    sprintf("./res_synth/case1_seq/datasets/random_seed%d.csv",seed),
                    row.names = FALSE)
        }
...
save(df_time, file="./res_synth/case1_seq/compiled_time.RData")
```

```{r SLGP_visu_seq, message=FALSE, warning=FALSE}
load(file="./res_synth/case1_seq/compiled.RData")
name_map <- c(
  "W1" = "Integrated Wasserstein 1 (SLGP, Bin)",
  "H" = "Hellinger(SLGP, Bin)",
  "KL_EstTrue" = "Integrated Kullback-Leibler(SLGP, Bin)",
  "KL_TrueEst" = "Integrated Kullback-Leibler(Bin, SLGP)",
...
```{r xgboost, message=FALSE, warning=FALSE, eval=FALSE}
library(xgboost)

in_dir <- "./res_synth/case1_seq/datasets"
out_dir <- "./res_synth/case1_seq/xgboost"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

for (seed in seq(10)){
  full_df <- read.csv(file=sprintf("./res_synth/case1_seq/datasets/random_seed%d.csv",seed))
  for (step in c(10, 25, 50, 75, 100, 150, 200)) {
    df_train <- full_df[1:step, ]
    dtrain <- xgb.DMatrix(
      data = as.matrix(df_train[, "p"]),
      label = as.integer(df_train$output)
    )
    params <- list(
      eta = 0.01,
      max_depth = 1,
    ...
    ## -----------------------------
    t2 <- Sys.time()
    pred_vec <- predict(fit, as.matrix(seq(0.00, 1.0,, 101)))
    ...
    temp$pred <- as.vector(pred_vec)
    temp$truth <- NA
    for(i in seq(nrow(temp))){
      temp$truth[i] <- dbinom(x=temp$output[i], 
                              size=n_binom, 
                              prob=temp$p[i])
    }
    temp2 <- temp %>%
      ...
      summarise(W1 = sum(abs(estim_cdf - truth_cdf)),
                ...
                .groups="keep") %>%
      data.frame()
    temp2$n  <- nrow(modelLaplace@data)
    temp2$strat <- "pred_entropy"
    time_pred <- as.numeric( difftime(Sys.time(), t2, units = "secs"))
    ...
    res <- data.frame(seed=seed,
                      ...
                      method="xgboost",
                      design="random",
                      IKL_TrueEst = mean(temp2$KL_TrueEst),
                      IKL_EstTrue = mean(temp2$KL_EstTrue),
                      IW1 = mean(temp2$W1),
                      time_sec = list(laplace=fit_time,
                                      optim=0, 
                                      pred=time_pred))
    write.csv(res,
              sprintf("./res_synth/case1_seq/xgboost/summary_seed%s_n%d.csv",
                      seed, step),
              row.names = FALSE)
    write.csv(temp,
              sprintf("./res_synth/case1_seq/xgboost/pred_seed%s_n%d.csv",
                      seed, step),
              row.names = FALSE)
...
```{r xgboost_seq, message=FALSE, warning=FALSE, eval=FALSE}
library(xgboost)
...
for (seed in seq(10)){
  ...
  df <- df_init
  outfinal <- sprintf("./res_synth/case1_seq/xgboost/entropy_seed%d_n%d.csv",
                      seed, n_end)
  if(!(file.exists(outfinal))){
    for (step in seq(n_end+1)) {
      out <- sprintf("./res_synth/case1_seq/xgboost/entropy_seed%d_n%d.csv",
                     seed, step)
      dtrain <- xgb.DMatrix(
        data = as.matrix(df[, "p"]),
        label = as.integer(df$output)
      )
      params <- list(
        eta = 0.01,
        max_depth = 1,
      ...
      f_obj <- function(x, fit){
        temp <- predict(fit, as.matrix(x, ncol=1))
        return(-sum(temp*log(temp)))
      }
      ...
      t_new <-  rbinom(n=5, size=n_binom, prob=p_new)
      df <- rbind(df, data.frame(p=p_new, output=t_new))
      opt_time <- as.numeric(difftime(Sys.time(), t1, units = "secs"))
...
```{r SLGP_compil_seq_vs_others, message=FALSE, warning=FALSE, eval=FALSE}
library(ggplot2)
...
df2_time <- data.frame()
for(seed in seeds){
  for (step in c(10, 50, 100, 200)) {
    out1 <- sprintf("./res_synth/case1_seq/GP/summary_seed%s_step%d_n%d.csv",
                    seed, step, step)
    out2 <- sprintf("./res_synth/case1_seq/xgboost/summary_seed%s_n%d.csv",
                    seed, step)
    
    res_gp <- read.csv(out1)
    colnames(res_gp)[6:8] <- c("W1", "KL_EstTrue", "KL_TrueEst")
    res_xgb <- read.csv(out2)
    colnames(res_xgb)[6:8] <- c("KL_TrueEst", "KL_EstTrue", "W1")
    
    
    df2_time <- rbind(df2_time, 
                      data.frame(seed=seed,
                                 n=step,
                                 method="GP",
                                 strat="random",
                                 t_train = res_gp$fit_time_sec,
                                 t_optim = 0,
                                 t_pred = NA))
    df2_time <- rbind(df2_time, 
                      data.frame(seed=seed,
                                 ...
                                 method="xgboost",
                                 strat="random",
                                 t_train = res_xgb$time_sec.laplace,
                                 t_optim = res_xgb$time_sec.optim,
                                 t_pred = res_xgb$time_sec.pred))
    
    
    res_xgb <- res_xgb[, 1:9]
    colnames(res_xgb)[9] <- colnames(res_gp)[9]
  ...
  for (step in seq(n_end+1)) {
    out3 <- sprintf("./res_synth/case1_seq/xgboost/entropy_seed%d_n%d.csv",
                    seed, step)
    
    res_xgb2 <- read.csv(out3)
    colnames(res_xgb2)[6:8] <- c("KL_TrueEst", "KL_EstTrue", "W1")
    df2_time <- rbind(df2_time, 
                      data.frame(seed=seed,
                                 n=5+5*step,
                                 method="xgboost",
                                 strat="pred_entropy",
                                 t_train = res_xgb2$time_sec.laplace,
                                 t_optim = res_xgb2$time_sec.optim,
                                 t_pred = res_xgb2$time_sec.pred))
    res_xgb2 <- res_xgb2[, 1:9]
    colnames(res_xgb2)[9] <- colnames(res_gp)[9]
    df2 <- rbind(df2, res_xgb2)
  }
}

save(df2, file="./res_synth/case1_seq/compiled2.RData")
save(df2_time, file="./res_synth/case1_seq/compiled2_time.RData")
```


```{r SLGP_vs_others, message=FALSE, warning=FALSE}
library(SLGP)
load(file="./res_synth/case1_seq/compiled.RData")
load(file="./res_synth/case1_seq/compiled2.RData")
name_map <- c(
  "W1" = "Integrated Wasserstein 1 (Model, Bin)",
  "H" = "Hellinger(SLGP, Bin)",
  "KL_EstTrue" = "Integrated Kullback-Leibler(Model, Bin)",
  "KL_TrueEst" = "Integrated Kullback-Leibler(Bin, Model)",
  "TV" = "Total variation(Model, Bin)")
method_labs <- c(
  "SLGP" = "SLGP - random design",
  "GPflow_multiclass_GP"  = "Multi-class GP - random design",
  "xgboost"  = "XGboost - random design",
  "xgboost_ent"  = "XGboost - predictive entropy"
)
name_crit <- "KL_TrueEst"
name_strat <- "random"
df <- df %>%
  filter(nFreq == 100) %>%
  filter(strat == name_strat) %>%
  group_by(seed, n, strat, name) %>%
  summarise(integratedDist = mean(med), .groups = "drop") %>%
  group_by(n, strat, name) %>%
  summarise(
  ...
  filter(name %in%c(name_crit))%>%
  data.frame()

df$method<- "SLGP"
df2 <- df2 %>%
  select(-step)%>%
  mutate(method=ifelse(method=="xgboost" & design=="pred_entropy",
                       "xgboost_ent", method))%>%
  pivot_longer(-c("seed", "n", "method", "design", "fit_time_sec")) %>%
  group_by(n, method, design, name) %>%
  summarise(
    med = median(value),
    q25 = quantile(value, probs = 0.25),
    q75 = quantile(value, probs = 0.75),
...
df <- rbind(df[, c("n", "name", "method", "med", "q25", "q75")],
            df2[, c("n", "name", "method", "med", "q25", "q75")])

df %>% ggplot() +
  geom_line(aes(x = n, y = med, col = method, group = method)) +
  geom_ribbon(aes(x = n, ymin = q25, ymax = q75, fill = method),
              alpha = 0.2, lty = 2) +
  facet_wrap(. ~ name, labeller = 
               labeller(name = as_labeller(name_map)), 
             scales = "free_y") +
  theme_bw() +
  labs(
    y = "Integrated distance (log-scale)",
    x = "n",
    colour = "Model used",  # legend title for lines
    fill   = "Model used"   # legend title for ribbons
  )+
  scale_colour_discrete(labels = method_labs) +
  scale_fill_discrete(labels = method_labs)+
  scale_y_log10()

ggsave("./Figures/Case1_othermodels.pdf", width=6, height=3.5)
...
gp_pred <- read.csv(sprintf("./res_synth/case1_seq/GP/pred_seed%d_n%d.csv", 1, 200))
gp_pred <-gp_pred %>% pivot_longer(-p) %>%
  mutate(name = as.integer(gsub("prob_", "", name)))%>%
  rename(output=name,
         GP = value) %>%
  data.frame()
xgb_pred <- read.csv(sprintf("./res_synth/case1_seq/xgboost/pred_seed%d_n%d.csv", 1, 200))
xgb_pred <- xgb_pred %>%
  rename(XGboost = pred) %>%
  select(p, output, truth, XGboost)

slgp_pred <- predictSLGP_newNode(modelLaplace1, 
                                 xgb_pred[, c("p", "output", "truth")], 
                                 interpolateBasisFun = "nothing", 
                                 nIntegral=11, discrete = TRUE)
SLGP_val <- rowMeans(slgp_pred[, -c(1:3)])
slgp_pred <- slgp_pred[, c(1:3)]
slgp_pred$SLGP <- SLGP_val
grid_pred <- merge(slgp_pred, xgb_pred, by=c("p", "output", "truth"))
grid_pred <- merge(grid_pred, gp_pred, by=c("p", "output"))
...
grid_pred %>%
  ...
  scale_colour_manual(
    name   = "Model used",
    breaks = c("truth", "SLGP", "XGboost", "GP"), 
    labels = c(truth   = "Ground truth",
               SLGP  = "SLGP - random design",
               XGboost = "XGboost - random design",
               GP = "Multiclass GP - random design"),
    values=c(truth   = "grey",
             SLGP  = "navy",
             XGboost = "chartreuse3",
             GP    = "red"))+  
...
ggsave("./Figures/Case1_pannelsothermethods.pdf", width=8, height=2.5)

```


```{r executionTime, warning=FALSE, message=FALSE}
load(file="./res_synth/case1_seq/compiled_time.RData")
load(file="./res_synth/case1_seq/compiled2_time.RData")

df_time <- rbind(df_time, df2_time)


df_time %>%
  filter(n %in% c(10, 50, 100, 200),
         method %in% c("xgboost", "GP", "SLGP"),
         strat == "random") %>%
  ggplot(aes(x = factor(n), y = t_train, 
             fill = method, shape =method)) +
  geom_boxplot(
    alpha = 0.8,
    outlier.alpha = 0.3,
...
ggsave("./Figures/Time_Case1_othermodels.pdf", width=5, height=5)
...
df_time %>%
  ...
  filter(n %in% c(10, 50, 100, 200))%>%
  group_by(n, strat) %>%
  summarise(training = median(t_train, na.rm = TRUE),
            selection = median(t_optim, na.rm = TRUE),
            .groups = "drop"
  ) %>%
  pivot_longer(
    cols = c(training, selection),
    names_to = "time_component",
    values_to = "time_val"
  ) %>%
  ggplot(aes(x = factor(n), y = time_val, fill = time_component)) +
  geom_col(width = 0.75, color="navy") +
  facet_wrap(~ strat, nrow = 1, 
             labeller = labeller(strat = strat_labs)) +
  scale_fill_manual(
    values = c(
      "training"  = "lightgrey",
    ...
    labels = c(
      "training"  = "SLGP training",
      "selection" = "Selection of a new point"
    )
  ) +
  labs(
    x = "Training sample size",
    y = "Median computational time (s)",
    fill = "Step"
  ) +
  theme_bw(base_size = 14) +
  theme(
...
ggsave("./Figures/Time_Case1_SLGP.pdf", width=8, height=4)
...
```{r SLGP_visu_seq2, message=FALSE, warning=FALSE}
...
strat_labs <- c(
  "random" = "Random",
  "pred_entropy"  = "Predictive entropy",
  "posterior_var"  = "Distributional variance",
  "bald" = "BALD"
)
strat_vals <- c("random" = "solid",
                "pred_entropy"  = "dashed",
                "posterior_var" = "dotted",
                "bald"          = "twodash")
df %>%
  filter(name %in% c("W1")) %>%
  ...
  ggplot() +
  geom_line(aes(x = ntrain, y = med, col = strat, 
                group = strat, linetype = strat)) +
  geom_ribbon(aes(x = ntrain, ymin = q25, ymax = q75, fill = strat, 
                  colour = strat, linetype = strat),
  ...
  labs(
    ...
    x = "Sample size n",
    colour = "Acquisition strategy",  # legend title for lines
    linetype = "Acquisition strategy",  # legend title for linetype
    fill   = "Acquisition strategy"   # legend title for ribbons
  )+
  scale_colour_discrete(labels = strat_labs) +
  scale_fill_discrete(labels = strat_labs)+
  scale_linetype_manual(labels = strat_labs,
                        values = strat_vals)
...
grid_pred %>%
  ...
  ylab("Probability")+
  xlab("k")+
  geom_step(mapping=aes(x=output, y=value, 
                        col=name, lty=name), direction = "mid")+
  facet_wrap(n ~ p , 
             nrow=3, labeller = labeller(
    n = function(x) paste0("n[bin]==", x),
...
dir.create("./res_synth/case2_seq/datasets",
           recursive = TRUE,
           showWarnings = FALSE)
  ...
  for(strat in list_strat){
    ...
    for (step in seq(n_end)) {
      ...
      if((file.exists(out))){
        ...
        if(nrow(res$slgp@data) %in% c(10, 50, 100, 195)){
          if(nrow(res$slgp@data)==195){
            df_time <- rbind(df_time, 
                             data.frame(seed=seed,
                                        n=200,
                                        method="SLGP",
        ...
        if(step == n_end && strat == "random"){
          df_data <- rbind(res$slgp@data, 
                           data.frame(output=res$t_new, n=res$n_new, p=res$p_new))
          
          write.csv(df_data,
                    sprintf("./res_synth/case2_seq/datasets/random_seed%d.csv", seed),
                    row.names = FALSE)
        }
      }
    }
...
save(df_time, file="./res_synth/case2_seq/compiled_time.RData")
```
...
```{r executionTime2, warning=FALSE, message=FALSE}
load(file="./res_synth/case2_seq/compiled_time.RData")
...
ggsave("./Figures/Time_Case2_SLGP.pdf", width=8, height=4)
